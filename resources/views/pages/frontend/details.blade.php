        ...
        <section class="bg-gray-100 px-4 py-16">
          <div class="container mx-auto">
            ...
            <div class="flex overflow-x-auto mb-4 -mx-3">
              @foreach ($recomendations as $data)
              <div class="px-3 flex-none" style="width: 320px">
                <div class="rounded-xl p-4 pb-8 relative bg-white">
                  <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                    <img
                      src="{{ $data->galleries()->exists() ? Storage::url($data->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}"
                      alt=""
                      class="w-full h-full object-cover object-center"
                    />
                  </div>
                  <h5 class="text-lg font-semibold mt-4">{{ $data->name }}</h5>
                  <span class="">IDR {{ number_format($data->price) }}</span>
                  <a href="{{ $data->slug }}" class="stretched-link">
                    <!-- fake children -->
                  </a>
                </div>
              </div>
